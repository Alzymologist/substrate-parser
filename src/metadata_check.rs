...
impl CheckedMetadata {
    pub fn new(meta_v14: RuntimeMetadataV14) -> Result<Self, MetaVersionError> {
...
impl MetaInput {
    ...
    pub(crate) fn checked(self) -> Result<CheckedMetadata, MetaVersionError> {
        match self {
            Self::Checked(checked_metadata) => Ok(checked_metadata),
            Self::Raw(meta_v14) => CheckedMetadata::new(meta_v14),
        }
    }
}
